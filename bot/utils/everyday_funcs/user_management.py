...
from db import async_session, get_all_users_stmt, update_user_stmt
...
async def user_mgmt(dp: Dispatcher, day: datetime.date):
    res = await async_session.execute(get_all_users_stmt())
    users_rows = res.all()
    ...
    for user in users:
        ...
        update_tasks.append(async_session.execute(update_user_stmt(user)))
